...
module.exports = function generate (inputFilePath, options) {
  ...
  */
  ...
  options.index = options.index || path.join(__dirname, ".index");
  ...
  */
  ...
  let index;
  try {
    index = parseInt(fs.readFileSync(options.index), 10) + 1;
  } catch (ex) {
    index = 0;
  }
  fs.writeFileSync(options.index, String(index % pairings.length));
